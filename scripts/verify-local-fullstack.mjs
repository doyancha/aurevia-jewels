...
await check('Cloudinary media', async () => { pass(media.length === 48, `expected 48 images, got ${media.length}`); pass(media.every((image) => /^https:\/\/res\.cloudinary\.com\/[^/]+\/image\/upload\//.test(image.url)), 'non-Cloudinary image URL found'); pass(new Set(media.map((image) => image.url)).size === 48, 'image URL is not unique'); let failed = 0; for (const image of media) { let ok = false; for (let attempt = 0; attempt < 2 && !ok; attempt++) { try { const result = await fetch(image.url, { method: 'HEAD' }); ok = result.ok && (result.headers.get('content-type') || '').startsWith('image/'); } catch {} } if (!ok) failed++; } pass(failed === 0, `${failed}/48 Cloudinary URLs failed`); });
...
console.log(`Cloudinary media: ${media.length}/${media.length}`);
