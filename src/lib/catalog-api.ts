...
function validateCollection(value: unknown): ApiCollection { const item = value as Partial<ApiCollection>; if (item.image_url !== null && typeof item.image_url !== 'string') throw new Error('collection.image_url must be a string or null.'); return { name: stringValue(item.name, 'collection.name'), slug: stringValue(item.slug, 'collection.slug'), description: stringValue(item.description, 'collection.description'), image_url: item.image_url ?? null }; }
function validateProduct(value: unknown): ApiProduct {
  ...
  if (!category || typeof category.name !== 'string' || typeof category.slug !== 'string') throw new Error('product.category is invalid.');
  if (!Array.isArray(item.collections) || item.collections.some((c) => typeof c?.name !== 'string' || typeof c?.slug !== 'string')) throw new Error('product.collections is invalid.');
  ...
  if (!Array.isArray(item.images) || item.images.some((image) => typeof image?.url !== 'string' || typeof image?.alt_text !== 'string' || typeof image?.sort_order !== 'number' || typeof image?.is_primary !== 'boolean')) throw new Error('product.images is invalid.');
